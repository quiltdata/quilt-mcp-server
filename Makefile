...
help:
	...
	@echo "  make test-buckets-curl     - Test bucket tools via HTTP/curl"
	@echo "  make test-packaging-curl   - Test packaging tools via HTTP/curl"
	@echo "  make test-permissions-curl - Test permissions tools via HTTP/curl"
	@echo "  make test-governance-curl  - Test governance tools via HTTP/curl (requires admin)"
	@echo ""
	...
	@echo "  make docker-build-amd64  - Build Docker image for AMD64 (ECS compatible)"
	@echo "  make docker-push         - Build and push Docker image to ECR"
	@echo "  make docker-push-amd64   - Build and push AMD64 image to ECR"
	@echo "  make docker-push-dev     - Build and push development Docker image"
	@echo "  make docker-release      - Complete release: build AMD64, push, deploy to ECS, wait"
	@echo "  make docker-release-dev  - Development release to ECS"
	@echo "  make ecs-deploy          - Deploy specific image to ECS (requires IMAGE_URI=<uri>)"
	@echo ""
