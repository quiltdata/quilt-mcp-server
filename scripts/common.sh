...
show_quick_commands() {
    ...
    log_info "View Lambda logs: ./tests/check_logs.sh"
    log_info "Get access token: ./tests/get_token.sh"
    log_info "Test API:"
    echo -e "${BLUE}  curl -H 'Authorization: Bearer \$(./tests/get_token.sh)' -X POST ${api_endpoint} -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/list\",\"params\":{}}'${NC}"
}
