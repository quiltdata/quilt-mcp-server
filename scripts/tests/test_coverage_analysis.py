...
def load_thresholds() -> CoverageThresholds:
    ...
    if not COVERAGE_REQUIRED_YAML.exists():
        raise FileNotFoundError(f"Coverage thresholds config not found: {COVERAGE_REQUIRED_YAML}")
...
def validate_coverage(report: CoverageReport, thresholds: CoverageThresholds) -> Tuple[bool, List[str], Dict]:
    ...
    errors = []
    all_passed = True
    results = {'summary': {}, 'files': {}, 'passed': True, 'errors': []}
    ...
    if summary_coverage:
        ...
        for metric, threshold in summary_thresholds.items():
            ...
            summary_results[metric] = {'actual': actual, 'required': threshold, 'passed': passed}
            if not passed:
                all_passed = False
    ...
    for file_path in thresholds.files.keys():
        ...
        for metric, threshold in file_thresholds.items():
            ...
            passed = actual >= threshold
            file_results[metric] = {'actual': actual, 'required': threshold, 'passed': passed}
            if not passed:
                all_passed = False
...
class TestCoverageThresholds:
    ...
    def test_coverage_csv_exists(self, coverage_report):
        ...
        assert COVERAGE_CSV.exists(), (
            f"Coverage CSV not found: {COVERAGE_CSV}\nRun 'make coverage' first to generate coverage data."
        )
    ...
    def test_summary_coverage_meets_thresholds(self, coverage_report, thresholds):
        ...
        for metric, threshold in thresholds.summary.items():
            actual = summary_coverage.get(metric, 0.0)
            if actual < threshold:
                errors.append(f"{metric}: {actual:.1f}% < {threshold:.1f}% (required)")
    ...
    def test_file_coverage_meets_thresholds(self, coverage_report, thresholds):
        ...
        for file_path in thresholds.files.keys():
            ...
            for metric, threshold in file_thresholds.items():
                actual = file_coverage.get(metric, 0.0)
                if actual < threshold:
                    errors.append(f"{file_path} {metric}: {actual:.1f}% < {threshold:.1f}%")
...
def main():
    ...
    try:
        ...
        print(f"✅ Results written to {COVERAGE_RESULTS_YAML}")
    except Exception as e:
        print(f"⚠️  Failed to write results YAML: {e}")
    ...
    if all_passed:
        ...
        for error in errors:
            print(f"  ❌ {error}")
        print(f"\n💡 See detailed results in {COVERAGE_RESULTS_YAML}")
        return 1
