...
async def demo_unified_search():
    ...
    demo_queries = [
        ...
        {"query": "files larger than 100MB", "description": "Analytical search with size filters", "scope": "global"},
        {"query": "README files", "description": "Simple file search", "scope": "global"},
        {
            "query": "csv",
    ...
    for i, demo in enumerate(demo_queries, 1):
        ...
        try:
            result = await unified_search(
                query=demo['query'], scope=demo['scope'], target=demo.get('target', ''), limit=5, explain_query=True
            )
            ...
            if backend_status:
                ...
                for backend, status in backend_status.items():
                    ...
                    print(
                        f"   {status_emoji} {backend}: {status['status']} "
