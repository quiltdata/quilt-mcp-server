...
class EnterpriseGraphQLBackend(SearchBackend):
        ...
        try:
            ...
            if result.get("success"):
                ...
                if "404" in str(error_msg):
                    self._update_status(
                        BackendStatus.UNAVAILABLE, "GraphQL endpoint not available (likely not Enterprise catalog)"
                    )
                else:
    ...
    async def _search_bucket_objects(
        ...
        variables = {"bucket": bucket.replace('s3://', ''), "filter": object_filter, "first": limit}
    ...
    async def _search_package_contents(
        self, query: str, package_name: str, filters: Optional[Dict[str, Any]], limit: int
    ) -> List[SearchResult]:
    ...
    def _build_user_meta_filters(self, filters: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if filters.get('quality_score_min'):
            user_meta_filters.append({'path': 'quality_score', 'number': {'gte': filters['quality_score_min']}})
