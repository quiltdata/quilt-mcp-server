    ...
    try:
        ...
        summary = {
            "package_info": {
                ...
                "creation_date": package_metadata.get("quilt", {}).get("creation_date", datetime.now(timezone.utc).isoformat()),
                "metadata_template": metadata_template,
                "description": package_metadata.get("quilt", {}).get("description", "Data package created via Quilt MCP Server")
            },
            ...
            "structure": {
                ...
                "organization_type": "smart_hierarchy" if any(folder for folder in organized_structure.keys()) else "flat",
                "auto_organized": True
            },
...
def generate_package_visualizations(
    ...
    try:
        ...
        if all_sizes and len(all_sizes) > 5:
            ...
            ax.hist(sizes_mb, bins=min(20, len(sizes_mb)//2), alpha=0.7, 
        ...
        if visualizations:
            ...
            if "folder_structure" in visualizations:
                ...
                ax1.bar(range(len(folders)), counts, color=COLOR_SCHEMES.get(metadata_template, COLOR_SCHEMES["default"]))
            ...
            if "file_size_distribution" in visualizations:
                ...
                ax3.hist(sizes, bins=min(15, len(sizes)//3), alpha=0.7, 
                        color=COLOR_SCHEMES.get(metadata_template, COLOR_SCHEMES["default"])[0])
