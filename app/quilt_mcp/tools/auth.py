...
def _get_catalog_host_from_config() -> Optional[str]:
    ...
    try:
        ...
        if config and config.get('navigator_url'):
            parsed = urlparse(config['navigator_url'])
            return parsed.hostname
    except Exception:
        pass
    return None
...
def catalog_url(registry: str, package_name: Optional[str] = None, path: Optional[str] = None, catalog_host: Optional[str] = None) -> dict[str, Any]:
    ...
    try:
        ...
        if package_name:
            ...
            url_parts = [catalog_host.rstrip('/'), 'b', bucket, 'packages', package_name, 'tree', 'latest']
            if path:
        ...
        return {
            "status": "error",
            "error": f"Failed to generate catalog URL: {e}"
        }
...
def catalog_uri(registry: str, package_name: Optional[str] = None, path: Optional[str] = None, 
    ...
    try:
        ...
        return {
            "status": "error",
            "error": f"Failed to generate Quilt+ URI: {e}"
        }
...
def filesystem_status() -> dict[str, Any]:
    ...
    if result.get("home_writable"):
        result.update(
            ...
            tools_available=[
                "auth_status", "catalog_info", "catalog_name", "catalog_url", "catalog_uri", 
                "filesystem_status", "packages_list", "packages_search", "package_browse", "package_contents_search", 
                "package_create", "package_update", "bucket_objects_list", "bucket_object_info", "bucket_object_text",
        ...
        result.update(
            ...
            tools_available=[
                "auth_status", "catalog_info", "catalog_name", "catalog_url", "catalog_uri",
                "filesystem_status", "packages_list", "package_browse", "package_contents_search"
            ],
        ...
        result.update(
            ...
            tools_available=["auth_status", "catalog_info", "catalog_name", "catalog_url", "catalog_uri", "filesystem_status"]
        )
