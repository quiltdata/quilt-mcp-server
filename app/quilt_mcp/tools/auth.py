...
def test_permissions(bucket: str) -> dict[str, Any]:
    """Test permissions for a specific bucket with diagnostic information.
    
    Args:
        bucket: S3 bucket name to test
        
    Returns:
        Dict with detailed permission test results and suggested fixes.
    """
    try:
        ...
        if result.get("success"):
            # Enhance the result with diagnostic information
            access_summary = result.get("access_summary", {})
            operation_tests = result.get("operation_tests", {})
            
            # Determine package creation capability
            package_create = access_summary.get("can_write", False) and access_summary.get("can_list", False)
            
            # Generate suggested fixes based on missing permissions
            suggested_fixes = []
            if not operation_tests.get("list", False):
                suggested_fixes.append("Add s3:ListBucket permission for bucket access")
            if not operation_tests.get("read", False):
                suggested_fixes.append("Add s3:GetObject permission for reading files")
            if not operation_tests.get("write", False):
                suggested_fixes.append("Add s3:PutObject permission for creating packages")
            
            if not suggested_fixes:
                suggested_fixes.append("All permissions look good! You can create packages in this bucket.")
            
            return {
                "status": "success",
                "bucket": bucket,
                "bucket_read": operation_tests.get("read", False),
                "bucket_write": operation_tests.get("write", False), 
                "bucket_list": operation_tests.get("list", False),
                "package_create": package_create,
                "permission_level": result.get("permission_level", "unknown"),
                "suggested_fixes": suggested_fixes,
                "guidance": result.get("guidance", []),
                "next_steps": [
                    "Try: package_create_from_s3() if permissions look good",
                    "Try: aws_permissions_discover() to see all your bucket access",
                    "Try: bucket_recommendations_get() for alternative buckets"
                ] if package_create else [
                    "Try: bucket_recommendations_get() to find writable buckets",
                    "Check IAM permissions for this bucket",
            ...
            return {
                "status": "error",
                "bucket": bucket,
                "error": result.get("error", "Permission check failed"),
                "suggested_fixes": [
                    "Verify the bucket name is correct",
                    "Check your AWS credentials",
                    "Ensure you have access to this bucket"
                ]
            }
        ...
        return {
            "status": "error",
            "bucket": bucket,
            "error": f"Failed to test permissions: {e}",
            "suggested_fixes": [
                f"Verify bucket '{bucket}' exists and you have access",
                "Check your AWS credentials with: aws sts get-caller-identity",
                "Try: aws_permissions_discover() to see your accessible buckets"
            ]
        }
