...
def workflow_update_step(
    ...
    try:
        ...
        workflow["completed_steps"] = sum(1 for s in workflow["steps"] 
                                        if s["status"] == StepStatus.COMPLETED.value)
        workflow["failed_steps"] = sum(1 for s in workflow["steps"] 
...
def workflow_get_status(workflow_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        completed_steps = workflow["completed_steps"]
        failed_steps = workflow["failed_steps"]
        in_progress_steps = sum(1 for s in workflow["steps"] 
                              if s["status"] == StepStatus.IN_PROGRESS.value)
        pending_steps = sum(1 for s in workflow["steps"] 
...
def workflow_list_all() -> Dict[str, Any]:
    ...
    try:
        ...
        for workflow_id, workflow in _workflows.items():
            summary = {
                ...
                "progress": {
                    ...
                    "total_steps": workflow["total_steps"],
                    "percentage": round((workflow["completed_steps"] / workflow["total_steps"]) * 100, 1) 
                                if workflow["total_steps"] > 0 else 0
                },
        ...
        return {
            ...
            "active_workflows": sum(1 for w in workflows_summary 
                                  if w["status"] in ["created", "in_progress"]),
            "completed_workflows": sum(1 for w in workflows_summary 
