...
def _generate_package_metadata(
    ...
    if metadata_template == "ml":
        ...
        metadata["analytics"] = {"type": "business_analytics", "analysis_ready": True, "report_generated": True}
...
def package_create_from_s3(
    ...
    try:
        ...
        source_info = {
            ...
            "source_description": f"s3://{source_bucket}/{source_prefix}"
            if source_prefix
        ...
        confirmation_info = {
            ...
            "structure_preview": {
                folder: {"file_count": len(files), "sample_files": [f["Key"] for f in files[:3]]}
                for folder, files in organized_structure.items()
                if files
            },
