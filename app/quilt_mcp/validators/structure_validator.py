...
DISCOURAGED_PATTERNS = [
    "temp", "tmp", "backup", "old", "test", "_test", "cache"
]
...
def validate_package_structure(
    ...
    if total_files > 100:
        ...
        if max_files_in_folder > 50:
            warnings.append(f"Large number of files ({max_files_in_folder}) in single folder - consider further organization")
...
def validate_folder_structure(folder_path: str) -> Tuple[List[str], List[str]]:
    ...
    if re.search(r'[^a-zA-Z0-9/_-]', folder_path):
        warnings.append(f"Folder '{folder_path}' contains special characters - use only alphanumeric, dash, and underscore")
