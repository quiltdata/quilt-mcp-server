...
def test_version_consistency_across_all_functions():
    ...
    with patch("quilt_mcp.tools.buckets.get_s3_client", return_value=mock_client):
        ...
        mock_client.head_object.assert_called_with(
            Bucket=test_bucket, Key=test_key, VersionId=test_version_id
        )
        ...
        mock_client.generate_presigned_url.assert_called_with(
            "get_object",
            Params={"Bucket": test_bucket, "Key": test_key, "VersionId": test_version_id},
            ExpiresIn=3600
        )
...
@pytest.mark.parametrize("version_id,should_fail", [
...
@pytest.mark.parametrize("error_code,error_message,expected_in_error", [
