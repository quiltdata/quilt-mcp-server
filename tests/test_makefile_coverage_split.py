"""Tests for the coverage-tools Makefile target."""
...
def test_coverage_split_target_exists():
    """Test that the coverage-tools target exists in make.dev."""
    makefile_path = Path(__file__).parent.parent / "make.dev"
    content = makefile_path.read_text()
    
    # The target should exist
    assert "coverage-tools:" in content, "coverage-tools target not found in make.dev"


def test_coverage_split_target_runs_unit_and_integration_tests():
    """Test that coverage-tools target runs both unit and integration tests with coverage."""
    ...
    coverage_split_section = content.split("coverage-tools:")[1].split("\n\n")[0] if "coverage-tools:" in content else ""
    
    # Should run pytest for both unit and integration tests
    assert "pytest tests/" in coverage_split_section, "coverage-tools should run pytest on tests/"
    assert "--cov=quilt_mcp" in coverage_split_section, "coverage-tools should include coverage reporting"


def test_coverage_split_creates_expected_output_files():
    """Test that coverage-tools creates the expected output files in build/test-results/."""
    ...
    assert "build/test-results" in coverage_split_section, "coverage-tools should create build/test-results directory"
    
    # Should generate coverage XML files
    expected_files = ["coverage-unit.xml", "coverage-integration.xml", "coverage-summary.md"]
    for expected_file in expected_files:
        assert expected_file in coverage_split_section, f"coverage-tools should generate {expected_file}"


def test_coverage_split_uses_correct_test_markers():
    """Test that coverage-tools uses the correct test markers for unit/integration classification."""
    makefile_path = Path(__file__).parent.parent / "make.dev"
    content = makefile_path.read_text()
