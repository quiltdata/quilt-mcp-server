...
def test_workflow_progression_updates_status_and_next_steps():
    ...
    wo.workflow_step_add("wf-001", "fetch", "Fetch data")
    wo.workflow_step_add("wf-001", "transform", "Transform data", dependencies=["fetch"])

    # Start the first step
    start_result = wo.workflow_step_update("wf-001", "fetch", "in_progress")
    assert start_result["workflow_status"] == "in_progress"

    # Complete both steps
    wo.workflow_step_update("wf-001", "fetch", "completed")
    final_update = wo.workflow_step_update("wf-001", "transform", "completed")

    assert final_update["workflow_status"] == "completed"
    assert final_update["progress"]["percentage"] == 100.0

    status = wo.workflow_status_get("wf-001")
...
def test_workflow_template_apply_sets_dependencies_and_guidance():
    ...
    status = wo.workflow_status_get("wf-template")
    assert status["next_available_steps"] == ["discover-packages"]

    # Progress through the first step to unlock downstream dependencies
    wo.workflow_step_update("wf-template", "discover-packages", "completed")
    status_after_step = wo.workflow_status_get("wf-template")
