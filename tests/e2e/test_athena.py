...
pytestmark = pytest.mark.usefixtures("backend_mode", "requires_catalog")
...
class TestTabulatorWorkflow:
    ...
    def test_tabulator_complete_workflow(self):
        ...
        if not buckets_result.get("success"):
            error_msg = buckets_result.get("error", "Unknown error")
            if "tabulator_data_catalog not configured" in error_msg:
                pytest.skip("Tabulator catalog not configured in this environment")
            pytest.fail(f"tabulator_buckets_list failed: {error_msg}")
