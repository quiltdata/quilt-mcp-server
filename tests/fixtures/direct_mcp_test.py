    ...
    async def test_unified_search_architecture(self) -> Dict[str, Any]:
        ...
        if "unified_search" in self.tools:
            ...
            results["unified_search_tests"].append({
                "success": False,
                "error": "unified_search tool not found"
            })
...
async def main():
    ...
    try:
        ...
        handled_errors = sum(1 for test in error["error_tests"] if not test["success"] or 
                           (isinstance(test.get("result"), dict) and test["result"].get("success") == False))
        ...
        if successful_basic >= total_basic * 0.8: passed_categories += 1
        if successful_search >= total_search * 0.8: passed_categories += 1  
        if handled_errors >= total_errors * 0.8: passed_categories += 1
        if "error" not in perf and perf.get('successful_calls', 0) >= 4: passed_categories += 1
        if successful_unified >= max(1, total_unified * 0.8): passed_categories += 1
