    ...
    def _analyze_query_and_select_tools(self, query: str) -> List[MockToolCall]:
        ...
        if "search" in query_lower or "find" in query_lower:
            ...
            if "unified_search" in self.tools:
                tool_calls.append(MockToolCall("unified_search", {
                    "query": query,
                    "scope": "catalog",
                    "limit": 3,
                    "explain_query": True
        ...
        if "bucket" in query_lower:
            tool_calls.append(MockToolCall("bucket_objects_search", {
...
class MockLLMMCPTester:
    ...
    async def test_performance(self) -> Dict[str, Any]:
        ...
        print(f"   Simulating 5 concurrent LLM conversations...")
        start_time = time.time()
        ...
        successful_conversations = sum(1 for r in results if any(
            tc["result"].success for tc in r["tool_calls"]
        ))
...
async def main():
    ...
    try:
        ...
        print(f"   Key tools: {len(tools['key_tools_found'])}/{len(tools['key_tools_found']) + len(tools['missing_tools'])}")
        
        # Functionality tests summary
        basic = results["basic_functionality"]["basic_tests"]
        successful_basic = sum(1 for test in basic if any(
            tc["result"].success for tc in test["tool_calls"]
        ))
        
        search = results["search_functionality"]["search_tests"]
        successful_search = sum(1 for test in search if any(
            tc["result"].success for tc in test["tool_calls"]
        ))
        
        unified = results["unified_search_architecture"]["unified_search_tests"]
        successful_unified = sum(1 for test in unified if any(
            tc["result"].success for tc in test["tool_calls"]
        ))
        ...
        handled_errors = sum(1 for test in error_tests if 
                           not test["result"].success or 
                           (test["result"].success and isinstance(test["result"].content, dict) and 
                            test["result"].content.get("success") == False))
