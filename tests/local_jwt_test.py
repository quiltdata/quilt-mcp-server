...
def test_session_caching():
    ...
    print(f"✅ Session authenticated and cached")
    ...
    if not cached_session:
        print(f"❌ Session not found in cache!")
        return False
...
def test_frontend_token_with_backend_secret():
    ...
    print(f"Testing frontend token with backend secret...")
    ...
    try:
        ...
        print(f"✅ Frontend token validates with backend secret!")
        print(f"   User: {payload.get('sub')}")
        print(f"   Buckets: {len(payload.get('buckets', []))}")
        return True
    except pyjwt.InvalidSignatureError:
        print(f"❌ Signature verification FAILED")
        print(f"   This means frontend is using a DIFFERENT secret!")
        ...
        print(f"   Frontend must be using a completely different secret")
        return False
