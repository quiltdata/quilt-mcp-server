...
usage() {
    echo "Usage: $0 [build|test|run|validate|clean] [options]"
    echo ""
    echo "Commands:"
...
case $COMMAND in
    build)
        ...
        if [ "$VERBOSE" = true ]; then
            docker build -t "$IMAGE_NAME" -f src/build-docker/Dockerfile .
        else
            docker build -t "$IMAGE_NAME" -f src/build-docker/Dockerfile . > /dev/null
        fi
