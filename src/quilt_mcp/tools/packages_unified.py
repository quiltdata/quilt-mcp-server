...
def packages(action: Optional[str] = None, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    ...
    try:
        if action is None:
            ...
            return format_error_response(f"Unknown packages action: {action}")
    
    except Exception as exc:
        logger.exception(f"Error executing packages action '{action}': {exc}")
        return format_error_response(f"Failed to execute packages action '{action}': {str(exc)}")
