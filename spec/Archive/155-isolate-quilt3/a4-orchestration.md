...
**Acceptance**:
- [ ] All exceptions defined with proper inheritance
- [ ] Exception messages are descriptive
- [ ] 100% coverage on exception module

### Phase 1.2: Admin Availability Check

**Goal**: Improve admin availability checking with proper error types.

**Tasks**:
1. Update `is_admin_available()` to use new exceptions
2. Add `_get_admin_exceptions()` helper that returns exception types
3. Update error handling in all admin methods

**Tests**:
- Update existing admin availability tests
- Add tests for exception type checking

**Acceptance**:
- [ ] `is_admin_available()` returns bool consistently
- [ ] Admin methods raise proper exception types
- [ ] All tests pass
...
## Phase 6: Deprecation & Cleanup (Week 2, Day 5)

### Phase 6.1: Mark Old Methods as Deprecated

**Goal**: Add deprecation warnings to old getter methods.

**Tasks**:
1. Add `@deprecated` decorator to old methods:
   - `get_users_admin()`
   - `get_roles_admin()`
   - `get_sso_config_admin()`
   - `get_tabulator_admin()`
   - `get_quilt3_module()`
   - `get_config()`
2. Update docstrings with migration guidance
3. Add deprecation warnings in method bodies

**Example**:
```python
@deprecated("Use list_users() and other user management methods instead")
def get_users_admin(self) -> Any:
    """Get users admin module.

    DEPRECATED: This method is deprecated. Use the following methods instead:
    - list_users() - List all users
    - get_user(name) - Get specific user
    - create_user(...) - Create user
    # ... etc
    """
    warnings.warn(
        "get_users_admin() is deprecated. Use specific user management methods.",
        DeprecationWarning,
        stacklevel=2
    )
...
### Phase 6.2: Remove Unused Methods

**Goal**: Delete methods that were never used.

**Tasks**:
1. Delete `get_search_api()` (never used)
2. Decide on `is_authenticated()`:
   - Keep if useful as convenience method
   - Remove if redundant with `get_logged_in_url()`
...
2. Add migration guide for old code
...
4. Check for any deprecation warnings in tests
...
## Phase 8: Future Removal (Post-Release)

**Goal**: Remove deprecated methods after grace period.

**Timeline**: 2-3 releases after deprecation

**Tasks**:
1. Verify no external usage of deprecated methods
2. Remove deprecated methods
3. Remove internal helper methods if no longer needed
