...
async def test_search_directly():
    ...
    try:
        ...
        for i, test in enumerate(test_queries, 1):
            ...
            try:
                result = await engine.search(query=test["query"], scope=test["scope"], bucket=test["bucket"], limit=10)
...
async def test_elasticsearch_backend():
    ...
    try:
        ...
        # Try a simple search
        try:
            result = await backend.search(query="README.md", scope="file", bucket="", limit=10)
...
def test_search_tool():
    ...
    try:
        ...
        for i, test in enumerate(test_queries, 1):
            ...
            try:
                result = search_catalog(query=test["query"], scope=test["scope"], bucket=test["bucket"], limit=10)
