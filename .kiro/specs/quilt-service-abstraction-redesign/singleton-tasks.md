...
- [ ] Create `src/quilt_mcp/context/request_context.py` to make tests pass
- [ ] Implement RequestContext with fields: request_id, tenant_id, user_id, auth_service,
...
- [ ] Write tests for context exceptions in `tests/unit/context/test_exceptions.py`
...
- [ ] Run tests: `pytest tests/unit/context/ -v`
...
- [ ] Write tests for AuthService ABC in `tests/unit/services/test_auth_service.py`
- [ ] Write tests for abstract method requirements (get_session, is_valid, get_user_identity)
- [ ] Write tests that instantiation is possible (not a singleton)
- [ ] Write tests for session caching within a single instance
- [ ] Update `src/quilt_mcp/services/auth_service.py` to remove singleton pattern
- [ ] Remove module-level `_AUTH_SERVICE` variable
- [ ] Define AuthService as abstract base class with required methods
