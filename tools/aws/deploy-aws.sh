    ...
    deploy)
        ...
        if [ -n "$ECR_URI" ]; then
            ...
            if setup_ecr_registry; then
                ...
                ECR_URI=$("$PROJECT_ROOT/catalog-push/catalog-push.sh" push 2>/dev/null | tail -1)
                export IMAGE_URI="$ECR_URI"
...
EOF
    ...
    validate)
        log_info "🔍 Phase 4: Deploy-AWS validation (SPEC compliant: actual deployment + endpoint test)"
        log_info "Requirements: CDK deploy succeeds, ECS healthy, public endpoint responds, HTTPS configurable"
        
        # Install CDK dependencies
        log_info "Installing CDK dependencies..."
        cd "$PROJECT_ROOT"
        if ! uv sync --group deploy > /dev/null 2>&1; then
        ...
        cd "$SCRIPT_DIR"
        if ! uv run cdk synth --app "python app.py" --output /tmp/cdk-out-http > /dev/null 2>&1; then
            log_error "❌ CDK synthesis failed (HTTP-only mode)"
            exit 1
        fi
        
        # Test CDK synthesis with HTTPS enabled
        log_info "Testing CDK synthesis (HTTPS enabled)..."
        if ! ACM_CERT_ARN="arn:aws:acm:us-east-1:123456789012:certificate/test-cert" uv run cdk synth --app "python app.py" --output /tmp/cdk-out-https > /dev/null 2>&1; then
            log_error "❌ CDK synthesis failed (HTTPS mode)"
            exit 1
        fi
        
        # SPEC REQUIREMENT: Deploy and test actual endpoint
        log_info "Deploying to AWS ECS (SPEC requirement)..."
        DEPLOY_ARGS=()
        if [ "$VERBOSE" = true ]; then
            DEPLOY_ARGS+=("-v")
        fi
        ...
        if ! "$0" deploy "${DEPLOY_ARGS[@]}"; then
            log_error "❌ Deploy command failed"
            exit 1
        fi
        
        # SPEC REQUIREMENT: Validate deployed service
        log_info "Testing deployed endpoint (SPEC requirement)..."
        TEST_ARGS=()
        if [ "$VERBOSE" = true ]; then
            TEST_ARGS+=("-v")
        fi
        if ! "$0" test "${TEST_ARGS[@]}"; then
            log_error "❌ Endpoint test failed"
            exit 1
        fi
        
        log_success "✅ Deploy-AWS phase validation passed (SPEC compliant: deployed + tested)"
        ;;
